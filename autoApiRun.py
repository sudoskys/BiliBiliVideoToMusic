...
__file_name__ = 'autoApiRun'
__author__ = 'sudoskys'
__time__ = '2022/2/10 下午9:20'
__product_name__ = 'PyCharm'
__version__ = '2月102120'
...
class mLog:
    ...
    def __init__(self, roadName, logs):
        if not all([roadName, logs]):  # 当 arg1, arg2, arg3都不为空时all函数返回true
            ...
            self.logr = str(useTool().filesafer(os.getcwd() + '/' + roadName + '.log'))
            self.logC = logs
            # use usetool ,please
        ...
        if not data:
            ...
            if isinstance(data, dict):
                for n, u in enumerate(data):
                    n = dataPull().well(data.get(u).get("title"))
                    urls = 'https://www.bilibili.com/video/' + str(data.get(u).get("bvid"))
                    print("START===" + urls)
                    try:
                ...
                for n, u in enumerate(data):
                    n = dataPull().well(u.get("title"))
                    urls = 'https://www.bilibili.com/video/' + str(u.get("bvid"))
                    print("START===" + n)
                    try:
                        dealUrl(n, urls, push, sync)
                    except BaseException as arg:
...
task = useTool().rData("data/autoapi/queue_rank.yaml")
# 得到任务
if task:
    for i, k in enumerate(task):
        ath = useTool().rData(task.get(k))
        mian(lme, ath, k)
